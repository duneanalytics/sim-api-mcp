...
describe("Sim API Client", () => {
  ...
  describe("getEVMActivity", () => {
    const address = "0xActivityWallet";
    const baseExpectedUrl = `https://api.sim.dune.com/beta/activity/evm/${address}`;
    // TODO: uncomment once the Sim OpenAPI spec includes the activity definitions again
    ...
    it("should correctly append all parameters", async () => {
      const params = {
        chain_ids: ["10", "137"],
        limit: 25,
        after_block_number: 987654,
        after_timestamp: 1678881000,
        tx_hash: "0xActivityTxHash",
      };
      await SimApi.getEVMActivity(address, params);

      const expectedUrl = new URL(baseExpectedUrl);
      expectedUrl.searchParams.append("chain_ids", "10,137");
      expectedUrl.searchParams.append("limit", "25");
      expectedUrl.searchParams.append("after_block_number", "987654");
      expectedUrl.searchParams.append("after_timestamp", "1678881000");
      expectedUrl.searchParams.append("tx_hash", "0xActivityTxHash");
    ...
    it("should return the parsed JSON response for activity", async () => {
      fetchSpy.mockResolvedValueOnce({
        ok: true,
